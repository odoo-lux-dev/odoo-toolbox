...
export class QuickDomainsService {
  ...
  private quickDomainsSyncStorage = storage.defineItem<QuickDomain[]>(
    <StorageItemKey>`sync:${CHROME_STORAGE_QUICK_DOMAINS_KEY}`,
    {
      init: () => [
        {
          id: "active-records",
          name: t("services.quick_domains.active_records"),
          domain: '[["active", "=", true]]',
          sequence: 0,
        },
        {
          id: "all-records",
          name: t("services.quick_domains.all_records"),
   ...
   */
  watchQuickDomains(callback: WatchCallback<QuickDomain[] | null>): () => void {
   ...
   */
  watchQuickDomainsOrdered(callback: WatchCallback<QuickDomain[] | null>): () => void {
    return storage.watch<QuickDomain[]>(
      <StorageItemKey>`local:${CHROME_STORAGE_QUICK_DOMAINS_KEY}`,
      (newValue, oldValue) => {
        // Always sort by sequence before calling callback
        const sortedNewValue = newValue?.toSorted((a, b) => a.sequence - b.sequence) || null;
        const sortedOldValue = oldValue?.toSorted((a, b) => a.sequence - b.sequence) || null;
  ...
  async setQuickDomainsOrdered(domains: QuickDomain[]): Promise<void> {
    ...
    const sortedDomains = uniqueDomains.toSorted((a, b) => a.sequence - b.sequence);
  ...
  async addQuickDomain(domain: Omit<QuickDomain, "sequence">): Promise<void> {
    ...
    if (domains.some((d) => d.id === domain.id)) {
      throw new Error(t("services.quick_domains.exists", [domain.id]));
    }
  ...
  async updateQuickDomain(
    ...
    if (!domainExists) {
      throw new Error(t("services.quick_domains.not_found", [domainId]));
    }
  ...
  async deleteQuickDomain(domainId: string): Promise<void> {
    ...
    if (updatedDomains.length === domains.length) {
      Logger.warn(`Quick domain with ID "${domainId}" not found for deletion`);
      return;
    }
