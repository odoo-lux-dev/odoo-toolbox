...
export class UpdateService {
   ...
   */
  shouldShowUpdatePage(currentVersion: string, previousVersion: string): boolean {
    ...
    const relevantUpdates = MAJOR_UPDATES.filter((update) => {
      const isAfterPrevious = this.compareVersions(update.version, previousVersion) > 0;
      const isBeforeOrEqualCurrent = this.compareVersions(update.version, currentVersion) <= 0;
      return update.shouldShowUpdatePage && isAfterPrevious && isBeforeOrEqualCurrent;
    });
  ...
  getUpdateInfo(currentVersion: string): {
    ...
    const eligibleUpdates = MAJOR_UPDATES.filter((update) => {
      return (
        update.shouldShowUpdatePage && this.compareVersions(update.version, currentVersion) <= 0
      );
    });
    ...
    eligibleUpdates.sort((a, b) => this.compareVersions(b.version, a.version));
  ...
  getUpdatesInRange(fromVersion: string, toVersion: string) {
    return MAJOR_UPDATES.filter((update) => {
      const isAfterFrom = this.compareVersions(update.version, fromVersion) > 0;
      const isBeforeOrEqualTo = this.compareVersions(update.version, toVersion) <= 0;
