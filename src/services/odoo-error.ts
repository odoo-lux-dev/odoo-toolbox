...
export class OdooError extends Error {
  ...
  getSummary(): string {
    ...
    const contextStr = context ? ` (${context.model}.${context.method})` : "";
...
export function hasOdooErrorData(
  ...
  return error instanceof Error && "odooError" in error && error.odooError !== undefined;
}

/**
 * Factory function to create an OdooError from an error response
 */
export function createOdooError(errorResponse: unknown, rpcContext?: RpcContext): OdooError {
  if (
    errorResponse &&
  ...
  throw new Error(t("services.errors.invalid_odoo_error"));
}
