...
export class FavoritesService {
  ...
  async deleteFromFavorites(projectToDelete: string): Promise<void> {
    const favorites = await this.getFavoritesProjects();
    const updatedFavorites = favorites.filter((favorite) => favorite.name !== projectToDelete);
   ...
   */
  async renameFavorite(favoriteId: string, newDisplayName: string): Promise<void> {
    const favorites = await this.getFavoritesProjects();
   ...
   */
  async setProjectTaskUrl(projectName: string, taskUrl: string): Promise<void> {
    const favorites = await this.getFavoritesProjects();
    const updatedFavorites = favorites.map((favorite) => {
  ...
  async reorderFavorites(orderedFavorites: Favorite[]): Promise<void> {
    const favoritesWithSequence = orderedFavorites.map((favorite, index) => ({
      ...favorite,
      sequence: index,
    }));
