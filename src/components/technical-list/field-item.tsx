    ...
    >
      ...
      {(isDynamicCondition(field.debugInfo?.required) ||
        ...
        <div className="x-odoo-technical-list-info-conditional-note">
          ...
          <span>* indicates conditional behavior based on field state</span>
        </div>
      )}
      ...
      {field.debugInfo && (
        <div className="x-odoo-technical-list-info-debug-details">
          {[
            { key: "widget", label: "Widget", value: field.debugInfo.widget },
            {
              key: "relation",
              label: "Relation",
            ...
            },
            {
              ...
              condition: () => isDynamicCondition(field.debugInfo?.required),
            },
            {
              key: "readonly",
              label: "Readonly Condition",
              value: field.debugInfo.readonly,
              condition: () => isDynamicCondition(field.debugInfo?.readonly),
            },
            {
              key: "invisible",
              label: "Invisible Condition",
              value: field.debugInfo.invisible,
              condition: () => isDynamicCondition(field.debugInfo?.invisible),
            },
          ]
            ...
            .map((item) => (
              ...
              >
                <span className="x-odoo-debug-label">{item.label}</span>
                <span className="x-odoo-debug-value x-odoo-json">
                  {item.serialize ? JSON.stringify(item.value) : item.value}
                </span>
              </div>
            ))}
        </div>
