...
describe("isPythonDomain", () => {
  test("should detect Python domain format", () => {
    ...
    expect(isPythonDomain("[('state', 'in', ['draft', 'done'])]")).toBe(true);
  });
...
describe("Advanced py.js capabilities", () => {
  ...
  test("should handle Python conditional expressions", () => {
    const result = validateDomain("[('name', 'ilike', 'test' if True else 'fallback')]");
    expect(result.isValid).toBe(true);
  ...
  test("should validate complex Python expressions through validateDomain", () => {
    const result = validateDomain("[('score', '>=', 80 + 20), ('active', '=', not False)]");
    expect(result.isValid).toBe(true);
    expect(result.normalizedDomain).toBeDefined();
    expect(result.normalizedDomain).toEqual([
      ["score", ">=", 100],
  ...
  test("should handle nested list expressions", () => {
    ...
    expect(result.isValid).toBe(true);
    expect(result.normalizedDomain).toEqual([["state", "in", ["draft", "done"]]]);
  });

  test("should handle Python comparison chains", () => {
    const result = validateDomain("[('priority', '=', 1 if 0 < 1 < 2 else 0)]");
    expect(result.isValid).toBe(true);
  ...
  test("should handle negative numbers and float operations", () => {
    const result = validateDomain("[('balance', '<', -100.5), ('rate', '=', 1.5 * 2)]");
    expect(result.isValid).toBe(true);
    expect(result.normalizedDomain).toEqual([
      ["balance", "<", -100.5],
      ["rate", "=", 3],
...
describe("validateDomain", () => {
  ...
  test("should accept valid simple domain", () => {
    ...
    expect(validateDomainWithPython('[["state", "in", ["draft", "done"]]]')).toBe(true);
  ...
  test("should accept valid complex domains with logical operators", () => {
    expect(validateDomainWithPython('["|", ["name", "=", "test"], ["code", "=", "TEST"]]')).toBe(
      true,
    );
    expect(validateDomainWithPython('["&", ["active", "=", true], ["id", ">", 0]]')).toBe(true);
  ...
  test("should accept all valid operators", () => {
    ...
    operators.forEach((operator) => {
      expect(validateDomainWithPython(`[["field", "${operator}", "value"]]`)).toBe(true);
    });
  });
  ...
  test("should accept any string operators (let Odoo validate)", () => {
    expect(validateDomainWithPython('[["name", "invalid_op", "test"]]')).toBe(true);
    expect(validateDomainWithPython('[["name", "==", "test"]]')).toBe(true);
    expect(validateDomainWithPython('[["name", "contains", "test"]]')).toBe(true);
  });
...
describe("validateDomainWithPython", () => {
  test("should accept JSON domains", () => {
    expect(validateDomainWithPython('[["name", "=", "test"]]')).toBe(true);
    expect(validateDomainWithPython('[["active", "=", true]]')).toBe(true);
    expect(validateDomainWithPython('[["parent_id", "=", null]]')).toBe(true);
    expect(validateDomainWithPython("[]")).toBe(true);
    expect(validateDomainWithPython("")).toBe(true);
  });
  ...
  test("should accept Python domains", () => {
    expect(validateDomainWithPython("[('name', '=', 'test')]")).toBe(true);
    expect(validateDomainWithPython("[('active', '=', True)]")).toBe(true);
    expect(validateDomainWithPython("[('parent_id', '=', None)]")).toBe(true);
    expect(validateDomainWithPython("[('active', '=', True), ('name', '!=', False)]")).toBe(true);
  });
  ...
  test("should reject domains with invalid logical operators", () => {
    expect(validateDomainWithPython('[["name", "=", "test"], "AND", ["id", ">", 0]]')).toBe(false); // AND instead of &
    expect(validateDomainWithPython('[["name", "=", "test"], "OR", ["id", ">", 0]]')).toBe(false); // OR instead of |
  });
});
...
describe("getValidationErrors", () => {
  ...
  test("should return model errors", () => {
    ...
    expect(errors).toContain("services.validation.model_required");

    const invalidQuery2 = { ...validQuery, model: "Invalid-Model" };
    const errors2 = getValidationErrors(invalidQuery2);
    expect(errors2).toContain("services.validation.invalid_model_format");
  });
  ...
  test("should return multiple errors", () => {
    ...
    expect(errors).toContain("services.validation.model_required");
    expect(
      errors.some((error) => error.includes("services.validation.invalid_domain_format")),
    ).toBe(true);
    expect(errors).toContain("services.validation.invalid_ids_format");
    expect(errors).toContain("services.validation.invalid_limit");
    expect(errors).toContain("services.validation.invalid_offset");
  });
});
...
describe("validateJSON", () => {
  ...
  test("should reject empty objects", () => {
    const result = validateJSON("{}");
    expect(result.isValid).toBe(false);
    expect(result.error).toBe("services.validation.json_empty_not_allowed");
  });

  test("should reject arrays", () => {
    const arrays = ["[]", "[1, 2, 3]", '["a", "b", "c"]', '[{"name": "John"}]'];

    arrays.forEach((jsonString) => {
      const result = validateJSON(jsonString);
      expect(result.isValid).toBe(false);
      expect(result.error).toBe("services.validation.json_arrays_not_allowed");
    });
  });
  ...
  test("should reject primitive values", () => {
    ...
    primitives.forEach((jsonString) => {
      ...
      expect(result.isValid).toBe(false);
      if (jsonString === "null") {
        expect(result.error).toBe("services.validation.json_objects_only");
      } else {
        expect(result.error).toBe("services.validation.json_primitives_not_allowed");
      }
    });
  });
...
describe("validateFieldsExistence", () => {
  ...
  test("should accept valid fields", () => {
    const json = '{"name": "John", "email": "john@example.com", "active": true}';
  ...
  test("should reject single invalid field", () => {
    ...
    expect(result.error).toBe("services.validation.invalid_field");
  });
  ...
  test("should reject multiple invalid fields", () => {
    ...
    const result = validateFieldsExistence(json, mockFieldsMetadata);
    expect(result.isValid).toBe(false);
    expect(result.invalidFields).toEqual(["invalid_field", "another_invalid"]);
    expect(result.error).toBe("services.validation.invalid_field");
  });

  test("should accept mix of valid and reject only invalid fields", () => {
    const json = '{"name": "John", "invalid_field": "value", "email": "john@example.com"}';
    const result = validateFieldsExistence(json, mockFieldsMetadata);
    expect(result.isValid).toBe(false);
    expect(result.invalidFields).toEqual(["invalid_field"]);
  });
...
describe("validateRequiredFields", () => {
  ...
  test("should fail when required fields are missing", () => {
    ...
    expect(result.error).toContain("services.validation.missing_required_field");
  });
