...
describe("validateDomainStructure", () => {
  ...
  test("should accept valid simple conditions", () => {
    expect(validateDomainStructure([["name", "=", "test"]]).isValid).toBe(true);
    expect(validateDomainStructure([["id", ">", 0]]).isValid).toBe(true);
    expect(validateDomainStructure([["active", "=", true]]).isValid).toBe(true);
    expect(validateDomainStructure([["parent_id", "=", null]]).isValid).toBe(true);
  ...
  test("should accept logical operators", () => {
    expect(
      validateDomainStructure(["&", ["name", "=", "test"], ["active", "=", true]]).isValid,
    ).toBe(true);
    expect(
      validateDomainStructure(["|", ["name", "=", "test"], ["email", "=", "test"]]).isValid,
    ).toBe(true);
    expect(validateDomainStructure(["!", ["active", "=", false]]).isValid).toBe(true);
  });
  ...
  test("should reject invalid structures", () => {
    ...
    expect(validateDomainStructure([["name", "="]]).isValid).toBe(false);
    expect(validateDomainStructure([["name", "=", "test", "extra"]]).isValid).toBe(false);

    // Invalid field names
    expect(validateDomainStructure([["", "=", "test"]]).isValid).toBe(false);
    expect(validateDomainStructure([[null, "=", "test"]]).isValid).toBe(false);
    expect(validateDomainStructure([[123, "=", "test"]]).isValid).toBe(false);

    // Invalid operator types (must be string)
    expect(validateDomainStructure([["name", 123, "test"]]).isValid).toBe(false);
    expect(validateDomainStructure([["name", null, "test"]]).isValid).toBe(false);
    expect(validateDomainStructure([["name", "", "test"]]).isValid).toBe(false);

    // Invalid logical operators
    expect(validateDomainStructure(["invalid", ["name", "=", "test"]]).isValid).toBe(false);
    expect(validateDomainStructure(["AND", ["name", "=", "test"]]).isValid).toBe(false);
    ...
    const result1 = validateDomainStructure(["field", "any_operator", "value"]);
  ...
  test("should handle various value types", () => {
    ...
    expect(validateDomainStructure([["score", "<=", 3.14]]).isValid).toBe(true);

    // Boolean values
    expect(validateDomainStructure([["active", "=", true]]).isValid).toBe(true);
    expect(validateDomainStructure([["archived", "!=", false]]).isValid).toBe(true);

    // Null values
    expect(validateDomainStructure([["parent_id", "=", null]]).isValid).toBe(true);

    // Array values (for 'in' operator)
    expect(validateDomainStructure([["state", "in", ["draft", "done"]]]).isValid).toBe(true);
    expect(validateDomainStructure([["id", "not in", [1, 2, 3]]]).isValid).toBe(true);
  });
});
...
describe("validateDomainStructureDetailed", () => {
  test("should return success for valid domains", () => {
    ...
    expect(validateDomainStructure(["&", ["name", "=", "test"], ["active", "=", true]])).toEqual({
      isValid: true,
    });
  });
  ...
  test("should provide specific error for invalid field names", () => {
    ...
    expect(result1.error).toContain("field name must be a non-empty string");
  ...
  test("should provide specific error for invalid domain items", () => {
    ...
    expect(result.error).toContain("expected logical operator (&, |, !) or condition array");
    expect(result.error).toContain("found number");
  });
