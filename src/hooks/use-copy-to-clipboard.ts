...
const FEEDBACK_CLASS_MAP: Record<FeedbackVariant, Record<FeedbackType, string[]>> = {
  field: {
...
const resolveFeedbackVariant = (targetElement: HTMLElement): FeedbackVariant => {
...
const setFeedbackState = (
  ...
  targetElement.setAttribute("data-clipboard-original-class", originalClassName);
  targetElement.setAttribute("data-clipboard-original-cursor", originalCursor);
  ...
  if (variant === "action") {
    ...
    targetElement.setAttribute("data-clipboard-original-width", targetElement.style.width);
    targetElement.setAttribute("data-clipboard-original-min-width", targetElement.style.minWidth);
...
const clearFeedbackState = (targetElement: HTMLElement) => {
  const originalHTML = targetElement.getAttribute("data-clipboard-original") ?? "";
  const originalClassName = targetElement.getAttribute("data-clipboard-original-class");
  const originalCursor = targetElement.getAttribute("data-clipboard-original-cursor");
  const originalWidth = targetElement.getAttribute("data-clipboard-original-width");
  const originalMinWidth = targetElement.getAttribute("data-clipboard-original-min-width");
  const originalPaddingLeft = targetElement.getAttribute("data-clipboard-original-padding-left");
  const originalPaddingRight = targetElement.getAttribute("data-clipboard-original-padding-right");
  ...
  if (originalClassName !== null) {
    ...
    Object.values(FEEDBACK_CLASS_MAP).forEach((variantClasses) => {
      targetElement.classList.remove(...variantClasses.success, ...variantClasses.error);
    });
  }
...
export const useCopyToClipboard = () => {
  const activeFeedbacks = new Set<HTMLElement>();

  const showFeedback = (targetElement: HTMLElement, message: string, type: FeedbackType) => {
    if (activeFeedbacks.has(targetElement)) return;

    activeFeedbacks.add(targetElement);
    ...
    setFeedbackState(targetElement, message, type, originalHTML, originalClassName, originalCursor);

    setTimeout(() => {
      clearFeedbackState(targetElement);
      activeFeedbacks.delete(targetElement);
    }, FEEDBACK_DURATION_MS);
  };

  const copyToClipboard = async (value: string, targetElement: HTMLElement) => {
    if (activeFeedbacks.has(targetElement)) return;

    const success = await copyText(value);
    if (success) {
      showFeedback(targetElement, t("common.copied"), "success");
    } else {
      showFeedback(targetElement, t("common.failed_to_copy"), "error");
    }
  };
