...
const handleFavoriteClick = async (event: Event, projectName: string): Promise<void> => {
  ...
  projectsList.toSorted(sortProjects).forEach((element_1) => tableBody?.appendChild(element_1));
};
...
const updateSearchBar = () => {
  ...
  projectsSearchBar.classList.replace("o_sh_projects_search", "x-odoo-sh-projects-search");
  ...
  projectsSearchBarInput.addEventListener(
    ...
    debounce((event: InputEvent) => {
      ...
      for (const card of cards) {
        ...
        const subscriptionCode = (card.dataset.subscriptionCode || "").toLowerCase();
...
const updateProjectList = (favorites: Favorite[]): void => {
  ...
  const sortedProjects = projects.toSorted((a, b) => {
    const aIsFavorite = favorites.some((fav) => projectNames.get(a) === fav.name);
    const bIsFavorite = favorites.some((fav) => projectNames.get(b) === fav.name);
  ...
  for (const project of sortedProjects) {
    ...
    const tdProjectName = firstChild.parentElement?.querySelectorAll("td")[1] as
      | HTMLElement
      | undefined;
    ...
    if (firstChild.children.length === 0) {
      ...
      if (tdProjectName) renameProjectName(favorite.display_name, tdProjectName);
    } else if (
