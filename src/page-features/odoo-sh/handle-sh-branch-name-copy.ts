...
const addCopyIconToBranchTitle = (): void => {
  ...
  const branchTitle = document.querySelector("div.o_branch_header_title") as HTMLDivElement;
  ...
  copyIconContainer.onclick = async () => {
    const success = await copyText(branchName);
    if (success) {
...
const addCopyIconToBranchTitleBuildPage = (currentLine: HTMLDivElement): void => {
  ...
  if (!branchName || !buttonsRow || buttonsRow.querySelector(".x-odoo-sh-copy-branch-build-name"))
    return;

  const copyIconContainer = document.createElement("a");
  copyIconContainer.classList.add("btn", "btn-sm", "btn-light", "x-odoo-sh-copy-branch-build-name");
  const icon = document.createElement("i");
  icon.className = "fa fa-clone";
