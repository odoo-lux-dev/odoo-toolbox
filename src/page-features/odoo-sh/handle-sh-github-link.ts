...
const addGithubIconToBranchTitle = (): void => {
  const branchTitle = document.querySelector("div.o_branch_header_title h4") as HTMLHeadingElement;

  const githubCommandInput = document.querySelector(
    "input[name='git_command']",
  ) as HTMLInputElement;

  const githubRepoInfo = githubCommandInput.value.match(REGEX_GITHUB_REPO_FROM_GIT_URL)?.groups
    ?.repo_info;
