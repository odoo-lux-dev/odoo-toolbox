...
// Returns the "Print" submenu trigger when it is expanded (`aria-expanded="true"`) AND
// carries a print icon. This is the only submenu we enrich: any other expanded submenu
// (e.g. "Marketing") must not fire a report request.
...
const handleTechnicalReportsVersion17andAbove = async (targetNode: Element): Promise<void> => {
  ...
  // (Odoo cog menu when there is only one report), whether the mutation targets the
  // item itself or the menu/overlay wrapping it.
  ...
  if (singlePrintItem) {
    // Single print option (cog menu): enrich the item itself.
    ...
    // Legacy nested submenu: the menu is a descendant of the parent.
    printOptionsEntries =
