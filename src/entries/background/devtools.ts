 ...
 */
async function executeInContentScript(scriptId: string, params?: unknown | null): Promise<unknown> {
  ...
  function getOdooInfo(): {
    ...
    const serverInfoVersion = (window.odoo?.info || window.odoo?.session_info)?.server_version_info
      ?.slice(0, 2)
      .join(".")
      .replace(/^saas~/, "")
      .replace(/\.0$/, "");
  ...
  const getCurrentSearchValues = () => {
    ...
    try {
      const owlDebug = window.odoo?.__WOWL_DEBUG__ as Record<string, unknown>;
   ...
   */
  async function executeOdooAction(params: OdooActionParams): Promise<unknown> {
    const { version, majorVersion } = getOdooInfo();
    const odoo = window.odoo;
