...
import { getUpdateInfo } from "@/utils/update-manager"
import { Logger } from "@/utils/logger"
import { ActivationMethod } from "@/utils/types"
import { useState } from "preact/hooks"

export const App = () => {
  const currentVersion = browser.runtime.getManifest().version
  const updateInfo = getUpdateInfo(currentVersion)
      ...
      case "custom":
        if (
          ...
          try {
            ...
            setButtonStates((prev) => ({
              ...prev,
              [index]: { text: "Error occurred", icon: "❌", disabled: true },
            }))
            setTimeout(() => {
              setButtonStates((prev) => {
    ...
    <div className="update-card">
      ...
      {updateInfo.mainFeature && (
        <div className="update-feature-highlight">
          <div className="feature-icon">{updateInfo.mainFeature.icon}</div>
        ...
        updateInfo.activationMethods.length > 0 && (
          <div className="update-activation-guide">
            ...
            <div className="activation-methods">
              {updateInfo.activationMethods.map((method, index) => {
                const currentState = buttonStates[index]
                const displayText = currentState?.text || method.text
                const displayIcon = currentState?.icon || method.icon
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      !isDisabled &&
                      method.action &&
                      handleActivationMethodClick(method, index)
                    }
                    role={method.action ? "button" : undefined}
                    tabIndex={method.action && !isDisabled ? 0 : undefined}
                    onKeyDown={(e) => {
                      if (!isDisabled && method.action && e.key === "Enter") {
                        e.preventDefault()
                        handleActivationMethodClick(method, index)
                      }
                    }}
                  >
                    <span className="method-icon">{displayIcon}</span>
      ...
      {updateInfo.customSections && updateInfo.customSections.length > 0 && (
        ...
        <div className="update-highlights">
          ...
          <ul>
            {updateInfo.notes.map((note, index) => {
              ...
              const text = emojiMatch ? note.slice(emojiMatch[0].length) : note

              return (
                <li key={index} className="update-note-item">
                  {emoji && <span className="note-emoji">{emoji}</span>}
