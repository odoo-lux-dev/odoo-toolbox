...
export const WriteTab = () => {
    ...
    const handleWriteExecute = async () => {
        if (!rpcQuery.model) {
            showNotification("Model is required for write operation", "error", 10000)
            return
        }
        ...
        if (!writeData.trim()) {
            showNotification("Write data is required", "error", 10000)
            return
        }
        ...
        if (!fieldsValidation.isValid) {
            ...
            showNotification(richNotification, "error", 10000)
            return
        }
