...
export const CallMethodTab = () => {
    ...
    const handleCallMethod = async () => {
        if (!rpcQuery.model) {
            showNotification("Model is required for method call", "error", 10000)
            return
        }

        if (!callMethodName.value.trim()) {
            showNotification("Method name is required", "error", 10000)
            return
        }
        ...
        if (!idsToUse) {
            showNotification("Record IDs are required for method call", "error", 10000)
            return
        }
